    ...
    while( iplus != jplus )
    {
        if( iplus->depth < jplus->depth )
        {
            if( iplus->orientation )
                { if( *delta > (iplus->flow) ) { iminus = iplus; *delta = (iplus->flow); *xchange = 0; } }
            else if( iplus->pred->pred )
                { if( *delta > ((flow_t)1 - iplus->flow) ) { iminus = iplus; *delta = ((flow_t)1 - iplus->flow); *xchange = 0; } }
            iplus = iplus->pred;
        }
        else
        {
            if( !jplus->orientation )
                { if( *delta >= (jplus->flow) ) { iminus = jplus; *delta = (jplus->flow); *xchange = 1; } }
            else if( jplus->pred->pred )
                { if( *delta >= ((flow_t)1 - jplus->flow) ) { iminus = jplus; *delta = ((flow_t)1 - jplus->flow); *xchange = 1; } }
            jplus = jplus->pred;
        }
    }
