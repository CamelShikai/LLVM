...
void __cdecl signal_handler(int sig_num) {
...
void vnotify(const char *fmt, va_list ap, int must_exit) {
...
int path_exists(const char *path, int is_dir) {
  file_stat_t st;
  return path == NULL || (stat(path, &st) == 0 &&
                          ((S_ISDIR(st.st_mode) ? 1 : 0) == is_dir));
}

void verify_existence(char **options, const char *name, int is_dir) {
