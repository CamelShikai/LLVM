...
int __attribute__((noinline)) error_get_error_code(const error_t *err)
{   
    VIRTUALIZER_START
    daemon_assert(invariant(err));
	VIRTUALIZER_END 
...
const char * __attribute__((noinline)) error_get_desc(const error_t *err)
{   
    VIRTUALIZER_START
    daemon_assert(invariant(err));
	VIRTUALIZER_END
...
#ifndef NDEBUG
static int __attribute__((noinline)) invariant(const error_t *err)
{
	VIRTUALIZER_START
    if (err == NULL) {
        return 0;
