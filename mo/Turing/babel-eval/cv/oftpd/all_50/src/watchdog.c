...
void __attribute__((noinline)) watchdog_defer_watched(watched_t *watched)
{
    watchdog_t *w;
    VIRTUALIZER_START
...
void __attribute__((noinline)) watchdog_remove_watched(watched_t *watched)
{
    watchdog_t *w;
    VIRTUALIZER_START
    daemon_assert(invariant(watched->watchdog));
...
static void __attribute__((noinline)) insert(watchdog_t *w, watched_t *watched)
{
   /*********************************************************************
...
static void __attribute__((noinline)) delete(watchdog_t *w, watched_t *watched)
{
    VIRTUALIZER_START
    if (!watched->in_list) {
        return;
