...
#ifdef _PROTO_
long __attribute__((noinline)) resize_prob( network_t *net )
#else
long __attribute__((noinline)) resize_prob( net )
     network_t *net;
#endif
{
...
#ifdef _PROTO_
void __attribute__((noinline)) insert_new_arc( arc_t *new, long newpos, node_t *tail, node_t *head,
                     cost_t cost, cost_t red_cost )
#else
void __attribute__((noinline)) insert_new_arc( new, newpos, tail, head, cost, red_cost )
     arc_t *new;
     long newpos;
     node_t *tail;
     node_t *head;
...
#ifdef _PROTO_
void __attribute__((noinline)) replace_weaker_arc( network_t *net, arc_t *new, node_t *tail, node_t *head,
                         cost_t cost, cost_t red_cost )
#else
void __attribute__((noinline)) replace_weaker_arc( net, new, tail, head, cost, red_cost )
     network *net;
     arc_t *new;
     node_t *tail;
     node_t *head;
...
#ifdef _PROTO_
long __attribute__((noinline)) price_out_impl( network_t *net )
#else
long __attribute__((noinline)) price_out_impl( net )
     network_t *net;
#endif
{
    long i;
...
#ifdef _PROTO_
long __attribute__((noinline)) suspend_impl( network_t *net, cost_t threshold, long all )
#else
long __attribute__((noinline)) suspend_impl( net, threshold, all )
     network_t *net;
     cost_t threshold;
     long all;
#endif
