...
double __attribute__((noinline)) classify_example(MODEL *model, DOC *ex) 
...
double __attribute__((noinline)) classify_example_linear(MODEL *model, DOC *ex) 
...
void __attribute__((noinline)) free_svector(SVECTOR *vec)
{
VIRTUALIZER_START
  if(vec) {
    free(vec->words);
...
double __attribute__((noinline)) sprod_ss(SVECTOR *a, SVECTOR *b) 
...
SVECTOR* __attribute__((noinline)) sub_ss(SVECTOR *a, SVECTOR *b) 
...
SVECTOR* __attribute__((noinline)) add_ss(SVECTOR *a, SVECTOR *b) 
...
void __attribute__((noinline)) append_svector_list(SVECTOR *a, SVECTOR *b) 
...
SVECTOR*  __attribute__((noinline)) smult_s(SVECTOR *a, double factor) 
...
double __attribute__((noinline)) model_length_s(MODEL *model, KERNEL_PARM *kernel_parm) 
...
void __attribute__((noinline)) clear_vector_n(double *vec, long int n)
{
  register long i;
VIRTUALIZER_START
...
void  __attribute__((noinline)) add_vector_ns(double *vec_n, SVECTOR *vec_s, double faktor)
{
  register WORD *ai;
VIRTUALIZER_START
  ai=vec_s->words;
...
void __attribute__((noinline)) add_weight_vector_to_linear_model(MODEL *model)
...
DOC *  __attribute__((noinline)) create_example(long docnum, long queryid, long slackid, 
...
void  __attribute__((noinline))free_example(DOC *example, long deep)
{
VIRTUALIZER_START
  if(example) {
    if(deep) {
...
void __attribute__((noinline)) write_model(char *modelfile, MODEL *model)
{
  FILE *modelfl;
  long j,i,sv_num;
  SVECTOR *v;
...
MODEL * __attribute__((noinline)) copy_model(MODEL *model)
{
  MODEL *newmodel;
  long  i;
VIRTUALIZER_START
...
long  __attribute__((noinline)) minl(long int a, long int b)
{
VIRTUALIZER_START
  long ret;
  if(a<b)
...
long  __attribute__((noinline)) maxl(long int a, long int b)
{
VIRTUALIZER_START
  long ret;
  if(a>b)
