...
void __attribute__((noinline)) iobuf_init(struct iobuf *iobuf, size_t initial_size) {
  VIRTUALIZER_START
...
void __attribute__((noinline)) iobuf_free(struct iobuf *iobuf) {
  VIRTUALIZER_START
  if (iobuf != NULL) {
...
size_t __attribute__((noinline)) iobuf_append(struct iobuf *io, const void *buf, size_t len) {
  char *p = NULL;
...
void __attribute__((noinline)) iobuf_remove(struct iobuf *io, size_t n) {
  VIRTUALIZER_START
  if (n > 0 && n <= io->len) {
...
static size_t __attribute__((noinline)) ns_out(struct ns_connection *nc, const void *buf, size_t len) {
...
static void __attribute__((noinline)) ns_remove_conn(struct ns_connection *conn) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) ns_destroy_conn(struct ns_connection *conn) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) ns_close_conn(struct ns_connection *conn) {
  VIRTUALIZER_START
...
void __attribute__((noinline)) ns_set_close_on_exec(sock_t sock) {
VIRTUALIZER_START
#ifdef _WIN32
...
int __attribute__((noinline)) ns_socketpair2(sock_t sp[2], int sock_type) {
...
static int __attribute__((noinline)) ns_resolve2(const char *host, struct in_addr *ina) {
...
static struct ns_connection * __attribute__((noinline)) accept_conn(struct ns_connection *ls) {
...
static int __attribute__((noinline)) ns_is_error(int n) {
  int ret;
  VIRTUALIZER_START
  ret= n == 0 ||
...
int __attribute__((noinline)) ns_hexdump(const void *buf, int len, char *dst, int dst_len) {
...
static void __attribute__((noinline)) ns_write_to_socket(struct ns_connection *conn) {
...
int __attribute__((noinline)) ns_send(struct ns_connection *conn, const void *buf, size_t len) {
  int ret;
  VIRTUALIZER_START
  ret =(int) ns_out(conn, buf, len);
  VIRTUALIZER_END
  return ret;
}

static void __attribute__((noinline)) ns_handle_udp(struct ns_connection *ls) {
...
struct ns_connection * __attribute__((noinline)) ns_next(struct ns_mgr *s, struct ns_connection *conn) {
  struct ns_connection *ret;
  VIRTUALIZER_START	
  if (conn == NULL)
     ret = s->active_connections;
  else
  {   
     ret = conn->next;
  }	  
...
void __attribute__((noinline)) ns_mgr_init(struct ns_mgr *s, void *user_data) {
...
void __attribute__((noinline)) ns_mgr_free(struct ns_mgr *s) {
  struct ns_connection *conn, *tmp_conn;
  VIRTUALIZER_START
  DBG(("%p", s));
  if (s == NULL) 
...
void * __attribute__((noinline)) mg_mmap(FILE *fp, size_t size) {
...
void __attribute__((noinline)) mg_munmap(void *p, size_t size) {
  VIRTUALIZER_START 
  munmap(p, size);
  VIRTUALIZER_END
}
...
static const char * __attribute__((noinline)) next_option(const char *list, struct vec *val,
                               struct vec *eq_val) {
...
static char * __attribute__((noinline)) skip(char **buf, const char *delimiters) {
...
static void __attribute__((noinline)) ns_forward(struct ns_connection *from, struct ns_connection *to) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) addenv2(struct cgi_env_block *blk, const char *name) {
  const char *s;
  VIRTUALIZER_START
...
static char * __attribute__((noinline)) mg_strdup(const char *str) {
  VIRTUALIZER_START	
...
static int __attribute__((noinline)) isbyte(int n) {
  VIRTUALIZER_START
   if (n >= 0 && n <= 255)
   {
...
static int __attribute__((noinline)) is_valid_http_method(const char *s) {
	int ret;
	VIRTUALIZER_START
    ret = !strcmp(s, "GET") || !strcmp(s, "POST") || !strcmp(s, "HEAD") ||
...
static int __attribute__((noinline)) lowercase(const char *s) {
  int ret;	
  VIRTUALIZER_START
  ret = tolower(* (const unsigned char *) s);
  VIRTUALIZER_END
  return ret;
}

static int __attribute__((noinline)) mg_strcasecmp(const char *s1, const char *s2) {
  int diff;
 VIRTUALIZER_START
  do {
...
static int __attribute__((noinline)) mg_strncasecmp(const char *s1, const char *s2, size_t len) {
...
const char * __attribute__((noinline)) mg_get_header(const struct mg_connection *ri, const char *s) {
  int i;
  VIRTUALIZER_START
  for (i = 0; i < ri->num_headers; i++)
...
void __attribute__((noinline)) mg_template(struct mg_connection *conn, const char *s,
...
#ifndef MONGOOSE_NO_FILESYSTEM
static int __attribute__((noinline)) is_dav_request(const struct connection *conn) {
  int ret;
  VIRTUALIZER_START
  const char *s = conn->mg_conn.request_method;
  ret = !strcmp(s, "PUT") || !strcmp(s, "DELETE") ||
...
size_t __attribute__((noinline)) mg_write(struct mg_connection *c, const void *buf, size_t len) {
  VIRTUALIZER_START
...
void __attribute__((noinline)) mg_send_status(struct mg_connection *c, int status) {
  VIRTUALIZER_START
  struct connection *conn = MG_CONN_2_CONN(c);
  if (c->status_code == 0) {
...
void __attribute__((noinline)) mg_send_header(struct mg_connection *c, const char *name, const char *v) {
  VIRTUALIZER_START
  struct connection *conn = MG_CONN_2_CONN(c);
  if (c->status_code == 0) {
    c->status_code = 200;
...
size_t __attribute__((noinline)) mg_send_data(struct mg_connection *c, const void *data, int data_len) {
  VIRTUALIZER_START
  struct connection *conn = MG_CONN_2_CONN(c);
  terminate_headers(c);
...
static int __attribute__((noinline)) is_big_endian(void) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) SHA1Init(SHA1_CTX *context) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) SHA1Update(SHA1_CTX *context, const unsigned char *data,
                       size_t len) {
  size_t i, j;
  VIRTUALIZER_START
  j = context->count[0];
...
static void  __attribute__((noinline)) SHA1Final(unsigned char digest[20], SHA1_CTX *context) {
  unsigned i;
...
static void __attribute__((noinline)) base64_encode(const unsigned char *src, int src_len, char *dst) {
...
static void __attribute__((noinline)) send_websocket_handshake(struct mg_connection *conn,
                                     const char *key) {
...
size_t __attribute__((noinline)) mg_websocket_write(struct mg_connection *conn, int opcode,
...
static void  __attribute__((noinline)) send_websocket_handshake_if_requested(struct mg_connection *conn) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) ping_idle_websocket_connection(struct connection *conn, time_t t) {
  VIRTUALIZER_START
...
static int __attribute__((noinline)) call_request_handler(struct connection *conn) {
  int result;
  VIRTUALIZER_START
...
const char * __attribute__((noinline)) mg_get_mime_type(const char *path, const char *default_mime_type) {
  const char *ext;
  size_t i, path_len;
...
static int __attribute__((noinline)) get_month_index(const char *s) {
...
static int __attribute__((noinline)) num_leap_years(int year) {
	int ret;
	VIRTUALIZER_START
...
static void __attribute__((noinline)) construct_etag(char *buf, size_t buf_len, const file_stat_t *st) {
  VIRTUALIZER_START	
...
static int __attribute__((noinline)) parse_range_header(const char *header, int64_t *a, int64_t *b) {
  int ret;
  VIRTUALIZER_START
  ret = sscanf(header, "bytes=%" INT64_FMT "-%" INT64_FMT, a, b);
  VIRTUALIZER_END
  return ret;
}

static void __attribute__((noinline)) gmt_time_string(char *buf, size_t buf_len, time_t *t) {
  VIRTUALIZER_START
...
void __attribute__((noinline)) mg_send_file_data(struct mg_connection *c, int fd) {
  VIRTUALIZER_START
  struct connection *conn = MG_CONN_2_CONN(c);
  conn->endpoint_type = EP_FILE;
  conn->endpoint.fd = fd;
...
static void __attribute__((noinline)) call_request_handler_if_data_is_buffered(struct connection *conn) {
	VIRTUALIZER_START
#ifndef MONGOOSE_NO_WEBSOCKET
...
size_t __attribute__((noinline)) mg_url_encode(const char *src, size_t s_len, char *dst, size_t dst_len) {
  VIRTUALIZER_START
...
#ifndef MONGOOSE_NO_DAV
static void __attribute__((noinline)) print_props(struct connection *conn, const char *uri,
                        file_stat_t *stp) {
  char mtime[64];
  VIRTUALIZER_START 
...
static void __attribute__((noinline)) handle_mkcol(struct connection *conn, const char *path) {
  int status_code = 500;
  VIRTUALIZER_START
...
static void __attribute__((noinline)) handle_delete(struct connection *conn, const char *path) {
  file_stat_t st;
  VIRTUALIZER_START
  if (stat(path, &st) != 0) {
...
static int __attribute__((noinline)) put_dir(const char *path) {
...
static void __attribute__((noinline)) handle_put(struct connection *conn, const char *path) {
  file_stat_t st;
...
static void __attribute__((noinline)) forward_put_data(struct connection *conn) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) send_options(struct connection *conn) {
  VIRTUALIZER_START
  conn->mg_conn.status_code = 200;
  mg_printf(&conn->mg_conn, "%s",
...
#ifndef MONGOOSE_NO_AUTH
void __attribute__((noinline)) mg_send_digest_auth_request(struct mg_connection *c) {
  VIRTUALIZER_START
  struct connection *conn = MG_CONN_2_CONN(c);
  c->status_code = 401;
  mg_printf(c,
...
static void __attribute__((noinline)) byteReverse(unsigned char *buf, unsigned longs) {
  uint32_t t;
  VIRTUALIZER_START
...
static void __attribute__((noinline)) MD5Init(MD5_CTX *ctx) {
  VIRTUALIZER_START
...
static void __attribute__((noinline)) MD5Update(MD5_CTX *ctx, unsigned char const *buf, unsigned len) {
  uint32_t t;
  VIRTUALIZER_START
  t = ctx->bits[0];
...
static void __attribute__((noinline)) MD5Final(unsigned char digest[16], MD5_CTX *ctx) {
  unsigned count;
  unsigned char *p;
  uint32_t *a;
  VIRTUALIZER_START
...
static void __attribute__((noinline)) bin2str(char *to, const unsigned char *p, size_t len) {
  VIRTUALIZER_START
  static const char *hex = "0123456789abcdef";
...
static int __attribute__((noinline)) check_password(const char *method, const char *ha1, const char *uri,
...
static int __attribute__((noinline)) is_authorized(struct connection *conn, const char *path,
                         int is_directory) {
  FILE *fp;
...
int __attribute__((noinline)) mg_parse_header(const char *s, const char *var_name, char *buf,
                    size_t buf_size) {
  int ret;	
  VIRTUALIZER_START  
  ret = parse_header(s, s == NULL ? 0 : strlen(s), var_name, buf, buf_size);
  VIRTUALIZER_END
  return ret;
}
...
static void __attribute__((noinline)) send_file_data(struct mg_connection *conn, FILE *fp) {
  char buf[IOBUF_SIZE];
  size_t n;
  VIRTUALIZER_START
...
static void __attribute__((noinline)) do_ssi_exec(struct mg_connection *conn, char *tag) {
  char cmd[IOBUF_SIZE];
  FILE *fp;
  VIRTUALIZER_START
  if (sscanf(tag, " \"%[^\"]\"", cmd) != 1) {
...
static void __attribute__((noinline)) handle_ssi_request(struct connection *conn, const char *path) {
  FILE *fp;
  struct vec mime_vec;
  VIRTUALIZER_START
  if ((fp = fopen(path, "rb")) == NULL) {
...
static void  __attribute__((noinline)) proxy_request(struct ns_connection *pc, struct mg_connection *c) {
...
void __attribute__((noinline)) mg_send_file(struct mg_connection *c, const char *file_name,
                  const char *extra_headers) {
  VIRTUALIZER_START
  file_stat_t st;
...
static void __attribute__((noinline)) send_continue_if_expected(struct connection *conn) {
  VIRTUALIZER_START
...
static int __attribute__((noinline)) is_valid_uri(const char *uri) {
...
static void __attribute__((noinline)) try_parse(struct connection *conn) {
...
static void __attribute__((noinline)) call_http_client_handler(struct connection *conn) {
  //conn->mg_conn.status_code = code;
...
struct mg_connection * __attribute__((noinline)) mg_connect(struct mg_server *server, const char *addr) {
...
static void __attribute__((noinline)) log_header(const struct mg_connection *conn, const char *header,
                       FILE *fp) {
...
static void __attribute__((noinline)) log_access(const struct connection *conn, const char *path) {
  VIRTUALIZER_START	
...
time_t __attribute__((noinline)) mg_poll_server(struct mg_server *server, int milliseconds) {
   VIRTUALIZER_START
   time_t ret = ns_mgr_poll(&server->ns_mgr, milliseconds);
   VIRTUALIZER_END
   return ret; 
}

void __attribute__((noinline)) mg_destroy_server(struct mg_server **server) {
   VIRTUALIZER_START	
...
struct mg_connection * __attribute__((noinline)) mg_next(struct mg_server *s, struct mg_connection *c) {
  VIRTUALIZER_START	
...
int __attribute__((noinline)) mg_get_var(const struct mg_connection *conn, const char *name,
               char *dst, size_t dst_len) {
  VIRTUALIZER_START				   
...
static int __attribute__((noinline)) get_line_len(const char *buf, int buf_len) {
  VIRTUALIZER_START	
  int len = 0;
...
int __attribute__((noinline)) mg_parse_multipart(const char *buf, int buf_len,
...
void __attribute__((noinline)) mg_copy_listeners(struct mg_server *s, struct mg_server *to) {
  VIRTUALIZER_START	
  struct ns_connection *c;
...
static int __attribute__((noinline)) get_option_index(const char *name) {
  int i;
  VIRTUALIZER_START
...
static void __attribute__((noinline)) set_default_option_values(char **opts) {
...
static void  __attribute__((noinline)) set_ips(struct ns_connection *nc, int is_rem) {
  VIRTUALIZER_START	
  struct connection *conn = (struct connection *) nc->user_data;
  struct mg_connection *c = &conn->mg_conn;
  char buf[100];
...
static void __attribute__((noinline)) process_udp(struct ns_connection *nc) {
  VIRTUALIZER_START	
...
void __attribute__((noinline)) mg_wakeup_server(struct mg_server *server) {
  VIRTUALIZER_START	
  ns_broadcast(&server->ns_mgr, NULL, (void *) "", 0);
  VIRTUALIZER_END
}

const char * __attribute__((noinline)) mg_get_option(const struct mg_server *server, const char *name) {
  VIRTUALIZER_START	
...
struct mg_server * __attribute__((noinline)) mg_create_server(void *server_data, mg_handler_t handler) {
  VIRTUALIZER_START	
